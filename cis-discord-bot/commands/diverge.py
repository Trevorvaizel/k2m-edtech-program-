...
def _record_showcase_publication(discord_id: str, decision: str, payload: Dict, post_text: str) -> None:
    ...
    try:
        store.create_showcase_publication(
            ...
            habits_demonstrated=payload.get("habits", ["PAUSE", "CONTEXT", "ITERATE"]),
            nodes_mastered=[],
...
def _build_showcase_post(author: discord.abc.User, decision: str, payload: Dict) -> str:
    ...
    return (
        f"{lead}{context_text}.\n"
        "Explored multiple possibilities in a private session.\n"
        "Practiced habits: Pause + Context + Iterate.\n"
...
async def handle_diverge(message: discord.Message, student):
    ...
    try:
        ...
        await dm_channel.send(
            "**ðŸ”„ The Explorer is taking a short break.**\n\n"
            ...
            "**You're practicing Habit 3 (ðŸ”„ ITERATE) - explore one direction at a time!**\n\n"
